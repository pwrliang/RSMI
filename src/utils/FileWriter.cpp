...
FileWriter::FileWriter(const std::string &prefix, const std::string &name) {
    this->prefix = prefix;
    this->name = name;
    file_utils::check_dir(prefix);
}

void FileWriter::write_mbrs(vector<Mbr> mbrs, ExpRecorder expRecorder) {
    ofstream write;
    string folder = Constants::WINDOW;
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + name + "_" + to_string(expRecorder.window_size) + "_" +
                to_string(expRecorder.window_ratio) + ".csv"), ios::out);
    for (Mbr mbr: mbrs) {
        write << mbr.get_self();
    }
    write.close();
...
void FileWriter::write_points(vector<Point> points, ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + name + ".csv"), ios::out);
...
void FileWriter::write_inserted_points(vector<Point> points, ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + name + "_" + to_string(expRecorder.insert_num) + ".csv"), ios::out);
    for (Point point: points) {
        write << point.get_self();
    }
    write.close();
...
void FileWriter::write_build(ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + expRecorder.structure_name + "_" + name + "_" + to_string(expRecorder.N) + ".txt"),
               ios::app);
...
void FileWriter::write_acc_window_query(ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + expRecorder.structure_name + "_" + name + "_" + to_string(expRecorder.window_size) +
                "_" + to_string(expRecorder.window_ratio) + "_" + to_string(expRecorder.N) + ".txt"), ios::app);
...
void FileWriter::write_acc_kNN_query(ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open((prefix + folder + expRecorder.structure_name + "_" + name + "_" + to_string(expRecorder.k_num) + "_" +
                to_string(expRecorder.N) + ".txt"), ios::app);
    write << expRecorder.get_time_pageaccess_accuracy();
    write.close();
}
...
void FileWriter::write_insert(ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open(
            (prefix + folder + expRecorder.structure_name + "_" + name + "_" + to_string(expRecorder.insert_num) + "_" +
             to_string(expRecorder.N) + ".txt"), ios::app);
...
void FileWriter::write_delete(ExpRecorder expRecorder) {
    ...
    file_utils::check_dir(prefix + folder);
    write.open(
            (prefix + folder + expRecorder.structure_name + "_" + name + "_" + to_string(expRecorder.delete_num) + "_" +
             to_string(expRecorder.N) + ".txt"), ios::app);
